...
class DDPG:
    ...
    def batch_to_tensor(self, batch):
        states = torch.tensor([self.normalize_state(s) for s in batch["states"]], dtype=torch.float, device=self.device)
        actions = torch.tensor(batch["actions"], dtype=torch.float, device=self.device)
        next_states = torch.tensor([self.normalize_state(s) for s in batch["next_states"]], dtype=torch.float, device=self.device)
